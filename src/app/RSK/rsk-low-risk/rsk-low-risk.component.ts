...
@Component({
  selector: 'app-rsk-low-risk',
  templateUrl: './rsk-low-risk.component.html',
  styleUrls: ['./rsk-low-risk.component.scss']
})
export class RskLowRiskComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      await this.allowance(this.bundleInstance,this.userAccount, environment.RSK.lowRiskAddress);
  ...
  fetchPrices() {
    this.RSKService.fetchRealPrices().subscribe((data: any) => {
  ...
  async fetchBundle(instance, userAccount) {
    await this.RSKService.fetchBundle(instance, userAccount, this.bundleId).then(async(data: any) => {
      if (data) {
        let date: any = new Date();
        date = date.getTime() / 1000;
        date = Math.round(date);

        console.log('-----------------| || ',data._staking_ends);
        console.log('---------date--------| || ',date);
        if (data && data._staking_ends) {
          // 2 TIMER :
          await this.ConvertSectoDayStackEnd(parseInt(data._staking_ends) - date);
        }
  ...
  leaderBoard(instance, userAccount, id) {
    this.RSKService.leaderBoard(id).subscribe(async (data: any) => {
  ...
  async onSubmitApprove() {
    ...
    if (this.approveForm.invalid || this.approveForm.value.amount == 0) {
      ...
      this.disabled = 'YES';
      this.spinner.show();
      await this.RSKService.approve(this.bundleInstance, environment.RSK.lowRiskAddress, this.approveForm.value.amount, this.userAccount).then(async (approveData) => {
        this.disabled = 'NO';
        this.spinner.hide();
  ...
  async onSubmitPlaceBet() {
    ...
    if (this.stakeForm.invalid || this.stakeForm.value.percentage == "0") {
      ...
      if (this.show && this.show == 'stake') {
        ...
        await this.RSKService.PlaceBet(this.fetchInstance, this.userAccount, this.approveDataObj, tempObj).then(async (stakeData) => {
          this.spinner.hide();
          this.disabled = 'NO';
          if (stakeData) {
            ...
            await this.RSKService.addStakeAPI(obj).subscribe((data: any) => {
              if (data && data.success) {
                this.toaster.success(data.message);
                this.router.navigate(['/rsk-bundle-asset']).then(() => {
                  window.location.reload();
                })
              }
            });
