...
@Component({
  selector: 'app-rsk-high-risk',
  templateUrl: './rsk-high-risk.component.html',
  styleUrls: ['./rsk-high-risk.component.scss']
})
export class RskHighRiskComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      this.fetchInstance = await this.RSKService.exportInstance(environment.RSK.highRiskAddress, environment.RSK.highRiskABI);
      ...
      await this.allowance(this.bundleInstance, this.userAccount, environment.RSK.highRiskAddress);
  ...
  fetchPrices() {
    this.RSKService.fetchRealPrices1().subscribe((data: any) => {
  ...
  async allowance(contractInstance, walletAddress, contractAddress) {
    await this.RSKService.allowance(contractInstance, walletAddress, contractAddress).then(async (data: any) => {
  ...
  async checkDisable() {
    await this.RSKService.getCompletedPoolData1(this.userAccount, this.bundleId).subscribe(async (data: any) => {
      if (data && data.success) {
  ...
  async onClickRegister() {
    ...
    if (this.registerForm.invalid) {
      ...
      this.spinner.show();
      this.RSKService.Register(this.fetchInstance, this.userAccount, this.registerForm.value.name).then((result) => {
        this.spinner.hide();
        if (result) {
          $('#regiter1').modal('hide');
          $('.modal-backdrop').hide();
  ...
  async onSubmitPlaceBet() {
    ...
    if (this.stakeForm.invalid || this.stakeForm.value.percentage == "0") {
      ...
      if (this.show && this.show == 'stake') {
        ...
        this.approveDataObj._prices = await this.RSKService.toWei(this.approveDataObj._prices);
        ...
        await this.RSKService.PlaceBet1(this.fetchInstance, this.userAccount, this.approveDataObj, tempObj).then(async (stakeData) => {
          this.spinner.hide();
          this.disabled = 'NO';
          if (stakeData) {
            ...
            await this.RSKService.addStakeAPI1(obj).subscribe((data: any) => {
              if (data && data.success) {
                this.toaster.success(data.message);
                this.router.navigate(['/rsk-bundle-asset1']).then(() => {
                  window.location.reload();
                })
              }
            });
  ...
  async onSubmitApprove() {
    ...
    if (this.approveForm.invalid || this.approveForm.value.amount == 0) {
      ...
      await this.RSKService.approve(this.bundleInstance, environment.RSK.highRiskAddress, this.approveForm.value.amount, this.userAccount).then(async (approveData) => {
        this.disabled = 'NO';
        this.spinner.hide();
  ...
  onClickClaimBund() {
    if (this.userAccount && this.userAccount != undefined) {
      ...
      this.RSKService.withdraw(this.fetchInstance, this.userAccount).then((data) => {
        this.spinner.hide();
  ...
  leaderBoard(instance, userAccount, id) {
    this.RSKService.leaderBoard1(id).subscribe(async (data: any) => {
      if (data && data.success && data.performance.length) {
        ...
        this.performance.map(async (x, i) => {
          // x = x.toObject();
          if (x && x.address) {
            x.user = await this.RSKService.fetchUser(instance, x.address);
          }
