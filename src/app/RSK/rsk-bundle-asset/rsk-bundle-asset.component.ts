...
@Component({
  selector: 'app-rsk-bundle-asset',
  templateUrl: './rsk-bundle-asset.component.html',
  styleUrls: ['./rsk-bundle-asset.component.scss']
})
export class RskBundleAssetComponent implements OnInit {
  ...
  async fetchUser(instance, userAccount) {
    await this.RSKService.fetchUser(instance, userAccount).then((data: any) => {
      if (data) {
        ...
        this.staked = data && data.staked_balance ? (data.staked_balance / environment.divideValue).toFixed(4) : 0;
