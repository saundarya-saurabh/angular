...
@Component({
  selector: 'app-rsk-liquidity-pool',
  templateUrl: './rsk-liquidity-pool.component.html',
  styleUrls: ['./rsk-liquidity-pool.component.scss']
})
export class RskLiquidityPoolComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      this.login = true;
      this.contractInstance = await this.RSKService.exportInstance(environment.RSK.poolAddress, environment.commanpoolABI);
      this.contractLpInstance = await this.RSKService.exportInstance(environment.RSK.ethLpAddress, environment.commanpoolABI);

      await this.balanceOf(this.userAccount, this.contractInstance, this.RSKService);
      await this.totalSupply(this.userAccount, this.contractInstance, this.RSKService)
      await this.earnedTokens(this.userAccount, this.contractInstance, this.RSKService);
      await this.allowance(this.contractLpInstance, this.userAccount, environment.RSK.poolAddress, this.RSKService);
  ...
  async balanceOfStaked() {
    if (this.userAccount) {
      ...
      let data: any = await this.RSKService.getBalance(instance, this.userAccount);
  ...
  async onClickApprove() {
    ...
    if (this.approveForm.invalid) {
      ...
      if (this.approveForm.value.amount == 0) {
        ...
        let address = environment.RSK.poolAddress;
        let instance = this.contractLpInstance;
  ...
  async onClickWithDraw() {
    ...
    if (this.withdrawForm.invalid) {
      ...
      let service: any = this.RSKService;
