...
export class LiquidityPoolComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      this.login = true;
      this.contractInstance = await this.apis.exportInstance(environment.poolAddress, environment.commanpoolABI);
      ...
      await this.allowance(this.contractLpInstance, this.userAccount, environment.poolAddress, this.apis);
