...
@Component({
  selector: 'app-completed-pools',
  templateUrl: './completed-pools.component.html',
  styleUrls: ['./completed-pools.component.scss']
})
export class CompletedPoolsComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      this.highRiskInstance = await this.apiService.exportInstance(environment.highRiskContractAddress, environment.highRiskABI);
  ...
  async fetchUser(instance, userAccount) {
    await this.apiService.fetchUser(instance, userAccount).then(async (data: any) => {
      if (data) {
        this.user = data;
        if (this.id == '1') {
  ...
  async showData(array) {
    ...
    await array.map(async (x, i) => {
      ...
      await this.apiService.getCompletedPoolData(this.userAccount, x).subscribe(async (data: any) => {
        if (data && data.success) {
          ...
          if (result.length) {
            ...
            // }
            // obj.completedArray = await this.generateReturnObj(result[0].bundleId, staked, performance, earn, result[0].prices);
  ...
  getPerformance(bundleId, address) {
    return new Promise(async (resolve, reject) => {
      this.apiService.getPerformance(bundleId, address).subscribe((data: any) => {
        if (data && data.success) {
          resolve(data)
        } else {
          resolve({
  ...
  async showDataHighrisk(array) {
    ...
    await array.map(async (x, i) => {
      ...
      await this.apiService.getCompletedPoolData1(this.userAccount, x).subscribe(async (data: any) => {
        if (data && data.success) {
          let result = await data['data'];
          if (result.length) {
            let obj: any = {
  ...
  claimBundInside(instance){
    ...
    this.apiService.withdraw(instance, this.userAccount).then((data) => {
      this.spinner.hide();
