...
export class HighRiskComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      this.fetchInstance = await this.apis.exportInstance(environment.highRiskContractAddress, environment.highRiskABI);
      ...
      await this.allowance(this.bundleInstance, this.userAccount, environment.highRiskContractAddress);
  ...
  async lastCreated(contractInstance, userAccount) {
    await this.apis.lastCreated(contractInstance, userAccount).then((data: any) => {
      if (data) {
  ...
  async fetchBundleId(instance, userAccount) {
    await this.apis.bundleId(instance, userAccount).then((data: any) => {
      if (data) {
  ...
  async fetchUser(instance, userAccount) {
    await this.apis.fetchUser(instance, userAccount).then((data: any) => {
      if (data) {
  ...
  async balanceOf(instance, userAccount) {
    await this.apis.getBalance(instance, userAccount).then((data: any) => {
      if (data) {
  ...
  fetchPrices() {
    this.apis.fetchRealPrices1().subscribe((data: any) => {
  ...
  async fetchBundle(instance, userAccount) {
    await this.apis.fetchBundle(instance, userAccount, this.bundleId).then((data: any) => {
      if (data) {
  ...
  async allowance(contractInstance, walletAddress, contractAddress) {
    await this.apis.allowance(contractInstance, walletAddress, contractAddress).then(async (data: any) => {
  ...
  async checkDisable() {
    await this.apis.getCompletedPoolData1(this.userAccount, this.bundleId).subscribe(async (data: any) => {
      if (data && data.success) {
  ...
  async onClickRegister() {
    ...
    if (this.registerForm.invalid) {
      ...
      this.spinner.show();
      this.apis.Register(this.fetchInstance, this.userAccount, this.registerForm.value.name).then((result) => {
        this.spinner.hide();
        if (result) {
          $('#regiter1').modal('hide');
          $('.modal-backdrop').hide();
          $("#regiter1").modal({ backdrop: 'static', show: false });
  ...
  async onSubmitPlaceBet() {
    ...
    if (this.stakeForm.invalid || this.stakeForm.value.percentage == "0") {
      ...
      if (this.show && this.show == 'stake') {
        ...
        this.approveDataObj._prices = await this.apis.toWei(this.approveDataObj._prices);
        ...
        await this.apis.PlaceBet1(this.fetchInstance, this.userAccount, this.approveDataObj, tempObj).then(async (stakeData) => {
          this.spinner.hide();
          this.disabled = 'NO';
          if (stakeData) {
            ...
            await this.apis.addStakeAPI1(obj).subscribe((data: any) => {
              if (data && data.success) {
                this.toaster.success(data.message);
                this.router.navigate(['/bundle-asset1']).then(() => {
                  window.location.reload();
                })
              }
            });
  ...
  async onSubmitApprove() {
    ...
    if (this.approveForm.invalid || this.approveForm.value.amount == 0) {
      ...
      await this.apis.approve(this.bundleInstance, environment.highRiskContractAddress, this.approveForm.value.amount, this.userAccount).then(async (approveData) => {
        this.disabled = 'NO';
        this.spinner.hide();
  ...
  onClickClaimBund() {
    if (this.userAccount && this.userAccount != undefined) {
      ...
      this.apis.withdraw(this.fetchInstance, this.userAccount).then((data) => {
        this.spinner.hide();
  ...
  leaderBoard(instance, userAccount, id) {
    this.apis.leaderBoard1(id).subscribe(async (data: any) => {
      if (data && data.success && data.performance.length) {
        ...
        this.performance.map(async (x, i) => {
          // x = x.toObject();
          if (x && x.address) {
            x.user = await this.apis.fetchUser(instance, x.address);
          }
