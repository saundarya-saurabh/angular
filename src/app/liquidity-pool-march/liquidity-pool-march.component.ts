...
@Component({
  selector: 'app-liquidity-pool-march',
  templateUrl: './liquidity-pool-march.component.html',
  styleUrls: ['./liquidity-pool-march.component.scss']
})
export class LiquidityPoolMarchComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.userAccount == undefined || !this.userAccount.length) {
      ...
      this.login = true;
      this.contractInstance = await this.apis.exportInstance(environment.poolAddressMAR, environment.commanpoolABI);
      ...
      await this.allowance(this.contractLpInstance, this.userAccount, environment.poolAddressMAR, this.apis);
  ...
  async onClickApprove() {
    ...
    if (this.approveForm.invalid) {
      ...
      if (this.approveForm.value.amount == 0) {
        ...
        let address = environment.poolAddressMAR;
        let instance = this.contractLpInstance;
